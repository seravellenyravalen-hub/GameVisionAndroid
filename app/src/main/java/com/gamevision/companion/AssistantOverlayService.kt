...
class AssistantOverlayService : Service(), TextToSpeech.OnInitListener {
    ...
    private fun togglePanel() {
        ...
        statusView = TextView(this).apply { text = "MANUAL • screen-aware chat"; textSize = 9f; setTextColor(0xFFB8FF3D.toInt()); typeface = android.graphics.Typeface.DEFAULT_BOLD; setPadding(0, 0, 0, dp(7)) }
    ...
    private fun sendCurrent() { val text = input?.text?.toString().orEmpty().trim(); if (text.isNotBlank()) { input?.setText(""); addMessage("user", text); ask(text) } }
    ...
    private fun ask(question: String) {
        ...
        executor.execute {
            try {
                ...
                connection.requestMethod = "POST"; connection.connectTimeout = 7000; connection.readTimeout = 25000; connection.doOutput = true
            ...
            } catch (e: Exception) { handler.post { addMessage("assistant", "Assistant unavailable: ${e.message ?: "connection error"}"); statusView?.text = "ERROR • try again" } }
        }
    }

    private fun startAuto() {
        val goal = input?.text?.toString().orEmpty().trim()
        if (goal.isBlank()) { addMessage("assistant", "Type or say the goal first, for example: ‘Play this game for me.’"); return }
        input?.setText("")
        addMessage("user", "AUTO: $goal")
        val snapshot = messages.takeLast(10).map { JSONObject().put("role", it.role).put("content", it.content) }
        val started = automation?.start(serverUrl, goal, snapshot) { status -> handler.post { statusView?.text = status; if (status.startsWith("AUTO OFF")) addMessage("assistant", status) } } == true
        if (!started) addMessage("assistant", "AUTO could not start. Make sure Accessibility is enabled for GameVision.")
    }
    ...
    private fun startVoice() {
        ...
        speechRecognizer = SpeechRecognizer.createSpeechRecognizer(this).apply {
            setRecognitionListener(object : RecognitionListener {
                ...
                override fun onResults(results: android.os.Bundle?) { val text = results?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)?.firstOrNull().orEmpty(); handler.post { if (text.isNotBlank()) { input?.setText(text); sendCurrent() } else statusView?.text = "NO SPEECH • try again" } }
                override fun onError(error: Int) { val message = voiceError(error); handler.post { statusView?.text = message; speechRecognizer?.destroy(); speechRecognizer = null } }
                override fun onRmsChanged(rmsdB: Float) {}; override fun onBufferReceived(buffer: ByteArray?) {}; override fun onEndOfSpeech() {}; override fun onPartialResults(partialResults: android.os.Bundle?) {}; override fun onEvent(eventType: Int, params: android.os.Bundle?) {}
            })
        }
        val intent = Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply { putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL, RecognizerIntent.LANGUAGE_MODEL_FREE_FORM); putExtra(RecognizerIntent.EXTRA_LANGUAGE, Locale.getDefault()); putExtra(RecognizerIntent.EXTRA_MAX_RESULTS, 1); putExtra(RecognizerIntent.EXTRA_PARTIAL_RESULTS, false) }
        runCatching { speechRecognizer?.startListening(intent) }.onFailure { statusView?.text = "VOICE START FAILED • try MIC again"; speechRecognizer?.destroy(); speechRecognizer = null }
    }
    ...
    private fun voiceError(error: Int): String = when (error) {
        ...
        SpeechRecognizer.ERROR_NO_MATCH -> "NO SPEECH • try again"
        SpeechRecognizer.ERROR_RECOGNIZER_BUSY -> "VOICE BUSY • retry in a moment"
        SpeechRecognizer.ERROR_SERVER -> "VOICE ERROR • recognition server"
        SpeechRecognizer.ERROR_SPEECH_TIMEOUT -> "NO SPEECH • speak after MIC"
