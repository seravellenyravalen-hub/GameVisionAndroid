...
class AssistantOverlayService : Service(), TextToSpeech.OnInitListener {
    ...
    private fun togglePanel() {
        ...
        statusView = TextView(this).apply { text = "CAPTURE • CHECKING"; textSize = 9f; setTextColor(0xFFB8FF3D.toInt()); typeface = android.graphics.Typeface.DEFAULT_BOLD; setPadding(0, 0, 0, dp(7)) }; root.addView(statusView)
    ...
    private fun pollFrameStatus() {
        ...
        executor.execute {
            ...
            try {
                ...
                if (code in 200..299 && body.isNotBlank()) { val json = JSONObject(body); val sequence = json.optInt("sequence", 0); val fresh = json.optBoolean("fresh", false); val age = json.optLong("ageMs", -1L); handler.post { statusView?.text = when { fresh && sequence > 0 -> "CAPTURE • LIVE • FRAME #$sequence • ${if (age >= 0) "${age}ms" else "fresh"}"; sequence > 0 -> "CAPTURE • STALE • FRAME #$sequence"; else -> "CAPTURE • WAITING FOR FIRST FRAME" } } } else handler.post { statusView?.text = "CAPTURE • SERVER $code" }
            } catch (_: Exception) { handler.post { statusView?.text = "CAPTURE • CONNECTION CHECK FAILED" } } finally { connection?.disconnect() }
        }
    }
    ...
    private fun startVoice() { voiceRetryCount = 0; startVoiceAttempt() }
    private fun startVoiceAttempt() {
        if (Build.VERSION.SDK_INT >= 23 && checkSelfPermission(Manifest.permission.RECORD_AUDIO) != PackageManager.PERMISSION_GRANTED) { addMessage("assistant", "Microphone permission is not enabled. Enable it in Android app permissions, then tap the mic icon again."); return }
        ...
        speechRecognizer?.apply { setRecognitionListener(object : RecognitionListener {
            override fun onReadyForSpeech(params: android.os.Bundle?) { handler.post { statusView?.text = "VOICE • LISTENING…" } }; override fun onBeginningOfSpeech() { handler.post { statusView?.text = "VOICE • LISTENING…" } }; override fun onRmsChanged(rmsdB: Float) { if (rmsdB > -2f) handler.post { statusView?.text = "VOICE • RECEIVING" } }; override fun onPartialResults(partialResults: android.os.Bundle?) { val text = partialResults?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)?.firstOrNull().orEmpty(); if (text.isNotBlank()) handler.post { input?.setText(text); input?.setSelection(input?.text?.length ?: 0); statusView?.text = "VOICE • $text" } }
            override fun onResults(results: android.os.Bundle?) { val candidates = results?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION).orEmpty(); val text = candidates.firstOrNull { it.isNotBlank() }?.trim().orEmpty(); handler.post { speechRecognizer?.destroy(); speechRecognizer = null; if (text.isNotBlank()) { voiceRetryCount = 0; input?.setText(text); input?.setSelection(text.length); sendCurrent() } else retryVoice("VOICE • NO SPEECH") } }
            override fun onError(error: Int) { val message = voiceError(error); handler.post { speechRecognizer?.destroy(); speechRecognizer = null; retryVoice(message) } }; override fun onBufferReceived(buffer: ByteArray?) {}; override fun onEndOfSpeech() { handler.post { statusView?.text = "VOICE • PROCESSING" } }; override fun onEvent(eventType: Int, params: android.os.Bundle?) {}
        }) }
        val intent = Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply { putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL, RecognizerIntent.LANGUAGE_MODEL_FREE_FORM); putExtra(RecognizerIntent.EXTRA_LANGUAGE, Locale.getDefault().toLanguageTag()); putExtra(RecognizerIntent.EXTRA_LANGUAGE_PREFERENCE, Locale.getDefault().toLanguageTag()); putExtra(RecognizerIntent.EXTRA_MAX_RESULTS, 3); putExtra(RecognizerIntent.EXTRA_PARTIAL_RESULTS, true); putExtra(RecognizerIntent.EXTRA_PROMPT, "Speak your GameVision command"); putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_COMPLETE_SILENCE_LENGTH_MILLIS, 1800L); putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_MINIMUM_LENGTH_MILLIS, 700L); putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_POSSIBLY_COMPLETE_SILENCE_LENGTH_MILLIS, 1400L) }
    ...
    private fun retryVoice(message: String) { if (voiceRetryCount < MAX_VOICE_RETRIES) { voiceRetryCount++; statusView?.text = "$message • RETRY $voiceRetryCount/$MAX_VOICE_RETRIES"; handler.postDelayed({ startVoiceAttempt() }, 700L) } else statusView?.text = "$message • TAP MIC TO TRY AGAIN" }
    ...
    override fun onDestroy() { handler.removeCallbacks(framePoller); automation?.stop("service destroyed"); automation?.shutdown(); removePanel(); bubble?.let { runCatching { wm?.removeView(it) } }; bubble = null; speechRecognizer?.destroy(); speechRecognizer = null; tts?.shutdown(); tts = null; executor.shutdownNow(); super.onDestroy() }
