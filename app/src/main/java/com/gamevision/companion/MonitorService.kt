...
class MonitorService : Service(), TextToSpeech.OnInitListener {
    ...
    private var serverUrl = "https://gamevision-monitor-0s07wr.v2.appdeploy.ai"
    ...
    private fun startProjection(intent: Intent) {
        ...
        serverUrl = intent.getStringExtra(EXTRA_SERVER_URL)?.takeIf { it.startsWith("http") } ?: serverUrl
        val resultCode = intent.getIntExtra(EXTRA_RESULT_CODE, Activity.RESULT_CANCELED)
        val data = intent.getParcelableExtraCompat<Intent>(EXTRA_RESULT_DATA) ?: return

        startForeground(NOTIFICATION_ID, notification("Monitoring visible game screen"), ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PROJECTION)
        ...
        reader?.setOnImageAvailableListener({ r -> onFrame(r) }, Handler(Looper.getMainLooper()))
        virtualDisplay = projection?.createVirtualDisplay(
            "GameVisionMonitor", width, height, metrics.densityDpi,
            DisplayManager.VIRTUAL_DISPLAY_FLAG_AUTO_MIRROR,
            reader?.surface, null, null
        )
        running.set(true)
        if (Settings.canDrawOverlays(this)) showOverlay("GameVision\nMonitoring…")
    }
    ...
    private fun onFrame(reader: ImageReader) {
        ...
        lastUploadAt = now
        executor.execute {
            try { uploadFrame(imageToJpeg(image)) } catch (_: Exception) { updateOverlay("GameVision\nFrame upload failed") } finally { image.close() }
        }
    }
    ...
    private fun imageToJpeg(image: Image): ByteArray {
        ...
        val scaledHeight = (cropped.height.toFloat() * scaledWidth / cropped.width).roundToInt().coerceAtLeast(1)
        ...
        bitmap.recycle(); if (cropped !== bitmap) cropped.recycle(); scaled.recycle()
    ...
    private fun uploadFrame(jpeg: ByteArray) {
        ...
        val body = "{\"image\":{\"data\":\"$base64\",\"mimeType\":\"image/jpeg\"}}"
        ...
        val score = analysis?.optString("score", "Visible frame") ?: "Visible frame"
        val confidence = analysis?.optInt("confidence", 0) ?: 0
        val note = analysis?.optJSONArray("notes")?.optString(0).orEmpty()
        val label = if (verified) "GameVision\n$score · $confidence%\nVERIFIED" else "GameVision\n$score · review"
        updateOverlay(label)
    ...
    private fun showOverlay(text: String) {
        ...
        windowManager = wm; overlay = view
    }

    private fun updateOverlay(text: String) { Handler(Looper.getMainLooper()).post { overlay?.text = text } }

    private fun toggleOverlay() {
        if (overlay == null) showOverlay("GameVision\nWaiting for frame…") else removeOverlay()
    }

    private fun removeOverlay() { overlay?.let { windowManager?.removeView(it) }; overlay = null }

    private fun speak(text: String) { Handler(Looper.getMainLooper()).post { tts?.speak(text, TextToSpeech.QUEUE_FLUSH, null, "gamevision") } }
    ...
    private fun stopProjection() {
        ...
        projection?.unregisterCallback(object : MediaProjection.Callback() {}, Handler(Looper.getMainLooper()))
    ...
    override fun onDestroy() { stopProjection(); tts?.shutdown(); executor.shutdownNow(); super.onDestroy() }
    ...
    private fun createNotificationChannel() {
        ...
        manager.createNotificationChannel(NotificationChannel(CHANNEL, "GameVision monitoring", NotificationManager.IMPORTANCE_LOW))
    }
}
