...
class AuthActivity : Activity() {
    private val serverUrl = "https://gamevision-api.onrender.com"
    ...
    private fun showAuth() {
        val root = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER_HORIZONTAL; setPadding(dp(24), dp(34), dp(24), dp(24)); setBackgroundColor(Color.rgb(7, 10, 14)) }
        val logo = TextView(this).apply { text = "G/V"; textSize = 24f; gravity = Gravity.CENTER; setTextColor(Color.BLACK); typeface = Typeface.DEFAULT_BOLD; background = rounded(0xFFB8FF3D.toInt(), 22) }; root.addView(logo, LinearLayout.LayoutParams(dp(82), dp(82)))
        val title = TextView(this).apply { text = "GameVision"; textSize = 30f; setTextColor(0xFFF4F7FA.toInt()); typeface = Typeface.DEFAULT_BOLD; gravity = Gravity.CENTER; setPadding(0, dp(18), 0, 0) }; root.addView(title)
        val subtitle = TextView(this).apply { text = "SEE • UNDERSTAND • REASON • ACT"; textSize = 10f; setTextColor(0xFF8D9AAA.toInt()); gravity = Gravity.CENTER; setPadding(0, dp(5), 0, dp(24)) }; root.addView(subtitle)
        ...
        val footer = TextView(this).apply { text = "Secure account • Free AI allowance • resets automatically"; textSize = 10f; setTextColor(0xFF596575.toInt()); gravity = Gravity.CENTER }; root.addView(footer)
        setContentView(root); updateCopy(heading, hint)
    }
    private fun updateCopy(heading: TextView, hint: TextView) { heading.text = if (createMode) "Create your account" else "Welcome back"; hint.text = if (createMode) "Create a GameVision account to use Live Vision and AI actions." else "Sign in to continue to your GameVision workspace."; action.text = if (createMode) "CREATE ACCOUNT" else "SIGN IN"; switchMode.text = if (createMode) "Already have an account?  Sign in" else "New to GameVision?  Create account"; status.text = "" }
    private fun submit() {
