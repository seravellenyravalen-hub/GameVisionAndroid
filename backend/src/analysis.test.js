...
test("normalizes generic visible screen elements", () => {
  const result = normalizeProviderResult(base, "openai");
...
test("marks strongly consistent generic vision as verified", () => {
  const result = mergeProviderResults(normalizeProviderResult(base, "openai"), normalizeProviderResult({ ...base, confidence: 88 }, "gemini"));
...
test("does not verify materially different screen states", () => {
  const result = mergeProviderResults(normalizeProviderResult(base, "openai"), normalizeProviderResult({ ...base, state: "gameplay", summary: "A racing track with a car" , confidence: 95 }, "gemini"));
